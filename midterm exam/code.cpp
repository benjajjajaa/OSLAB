...
enum class BB {READY, RUNNING, BLOCKED, TERMINATED }; //defines the possible states a process can be in during the simulation

struct CC {
    int DD;
    AA EE;      // process type/category. use to determine queue placement and priority.
    int FF;
    int GG;
    ...
    CC(int pid, AA t, int cpuTime, const vector<int>& maxRes)
        : DD(pid), EE(t), FF(cpuTime), GG(cpuTime), //Sets the process type. this assigns its category (SYSTEM, INTERACTIVE, or BATCH)
          HH(0), II(0), JJ(BB::READY), //initializes the process state to READY (ready to run)
...
public:
    ...
    bool PP(shared_ptr<CC> p, const vector<int>& request) {
        ...
        for (size_t i = 0; i < request.size(); ++i) {
            if (request[i] > available[i]) {
                p->JJ = BB::BLOCKED; //blocked because no available resource
                return false;
            }
        }
        ...
        p->JJ = BB::READY; //the process can now be scheduled to run
        return true;
    }
    ...
    vector<shared_ptr<CC>> RR() {
        ...
        for (auto& p : allProcesses) {
            if (p->JJ == BB::BLOCKED) { //checks if the process's state is BLOCKED
...
public:
    ...
    void OO(shared_ptr<CC> p) {
        p->HH = 0;
        p->II = 0;
        int typeIdx = YY(p->EE); //stores the result in typeIdx
        queues[typeIdx][0].push(p);
    }

    void UU(shared_ptr<CC> p) {
        int typeIdx = YY(p->EE); //used to access the correct set of priority queues
...
int main() {
    ...
    while (timeStep < maxTime) {
        ...
        current->JJ = BB::RUNNING; //process is now actively executing on the CPU for its allocated quantum.
        int quantum = ZZ.WW(current->HH);

        string typeStr = (current->EE == AA::SYSTEM) ? "SYSTEM" : // printed in the simulation output to display the process category in a readable format.
        ...
        if (current->GG == 0) {
            current->JJ = BB::TERMINATED; //terminated when remaining CPU time reaches 0. process has completed its execution.
            ...
            current->II = 0;
            current->JJ = BB::READY; //process to be re-queued. it still has remaining CPU time
            ZZ.UU(current);
        }

        rm.SS(); 

        bool allDone = (p1->JJ == BB::TERMINATED && //hecks if all three processes have completed execution
                        p2->JJ == BB::TERMINATED &&
                        p3->JJ == BB::TERMINATED);
